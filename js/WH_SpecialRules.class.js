...
бы одну модель с этим специальным правилом, получает бонус +1 к броскам при прохождении тестов на отрицание ведьмовства (с. 25).' 
}, {
    specialRuleName: 'AndTheyShallKnowNoFear',
    visibleName: 'And they shall know no fear',
    textEng : 'A unit that contains at least one model with this special rule automatically passes Fear
...
Advance, they are not destroyed, but remain locked in combat instead.',
    ...
    textRus : 'Боевая единица, содержащая хотя
бы одну модель с этим специальным правилом, автоматически проходит тесты на устрашение и перегруппировку. При перегруппировке боевая единица
не делает манёвр на 3", однако вместо этого может двигаться, стрелять (или бегать) и объявлять о нападении как обычно в этом ходе. Если боевую единицу, содержащую одну или более моделей с этим специальным правилом, настигают при преследовании, то она
не уничтожается, а остаётся связанной боем.'
}, {
    specialRuleName: 'Armourbane',
    visibleName: 'Armourbane ',
    textEng : 'If a model has this special rule, or is attacking with a Melee weapon that has this special
...
non-vehicle models..',
    ...
    textRus : 'Если модель имеет это специальное правило или атакует рукопашным оружием с этим специальным правилом, то она кидает дополнительный D6 на пробитие брони в ближнем бою.
Аналогичным образом, если модель совершает стрелковую атаку оружием с этим специальным правилом, она кидает дополнительный D6 на пробитие брони. В любом случае данное специальное правило не действует на модели, не относящиеся к технике.'
}, {
    ...
    textEng : 'Passengers disembarking from Access Points on a vehicle with this special rule can charge
on the turn they do so (even in a turn that the vehicle was destroyed, or in the following',    
    ...
    visibleName: 'Barrage',
    textEng: 'All Barrage weapons use blast markers and consequently use the rules for Blast weapons,
...
applicable). When firing indirectly, the Ballistic Skill of the firer is not subtracted from
the scatter distance; unless a Hit! is rolled on the scatter dice, the blast marker always
scatters a full 2D6". If a Barrage weapon has line of sight to its target it can fire directly,
even if the target is within its minimum range.
...
previous marker).
• Once all of the markers are in place, add up the number of hits and roll To Wound for
these hits. To determine Wound allocation and cover saves, always assume the shot is
coming from the centre of the first blast marker that was placed in the Multiple Barrage.
Apocalyptic Barrage
An Apocalyptic Barrage follows all of the rules for a Barrage weapon, but uses the clovershaped
apocalyptic barrage marker. Before the marker is placed, the attacker is allowed to
rotate the marker about its centre to maximise the number of models that could
potentially be hit. Place the marker and roll for scatter in the same way you would for a
Blast. If the marker scatters, be careful to maintain the same orientation as you move it.
Once the final position of the marker has been determined, roll a number of dice equal to
the number of attacks on the weapon’s profile. So, for example, with a weapon with the
type ‘Heavy 4, Apocalyptic Barrage’, you would roll four dice. Each dice roll corresponds to
a ‘strike’ on one of the circles on the apocalyptic barrage marker. For example, each roll of
a 2 indicates a strike on circle number 2. Resolve the strikes as for a Multiple Barrage, as
if each were a Barrage attack that had landed on that circle and hit all the models
underneath it. To determine Wound allocation and cover saves, always assume the shot is
coming from the centre of the entire apocalyptic barrage marker.',
    ...
    visibleName: 'Blast',
    textEng : 'When firing a Blast weapon, models do not roll To Hit. Instead, just pick one enemy
...
Large Blast
Large Blast weapons use the 5" blast marker, but otherwise obey all the rules for Blast
weapons.
Massive Blast
Massive Blast weapons use the 7" blast marker, but otherwise obey all the rules for Blast
weapons.
Apocalyptic Blast
Apocalyptic Blast weapons use the 10" blast marker, but otherwise obey all the rules for
Blast weapons.
Apocalyptic Mega-blast (5"/7"/10")
Apocalyptic Mega-blast weapons use the apocalyptic blast marker. They obey the rules for
Blast weapons, with the following exceptions:
• Apocalyptic Mega-blast weapons have three Strength values and three AP values.
Correspondingly, the apocalyptic blast marker is divided into three zones, as shown in
the diagram to the far right, one for each Strength and AP value.
• The Strength and AP of any hits depends on the zone where the target model is
located. The first Strength and AP value are used for the inner zone, the second Strength
and AP value are used for the middle zone, and the third Strength and AP value are used
for the outer zone. Always use the best Strength and AP if a model straddles two or more
zones. If a unit has models in several zones, work out the hits inflicted for each zone
separately. Note that there will be a different Wound pool for each zone.
For example, an Aquila macro-cannon firing a quake shell has the Apocalyptic Mega-blast
special rule, a Strength of 10/7/5 and an AP value of 1/4/6. The Strength and AP values
for the three zones are therefore:
Zone - S - AP
Inner - 10 - 1
Middle - 7 - 4
Outer - 5 - 6
...
• To determine whether a unit wounded by an Apocalyptic Mega-blast weapon is
allowed a cover save, and when determining Wound allocation, always assume the shot
is coming from the centre of the apocalyptic blast marker, instead of from the firing
model.
• Hits from Apocalyptic Mega-blast weapons made against vehicles are always resolved
against their side armour.',
    nameRus : 'Взрыв',    
    textRus : 'При стрельбе из оружия взрывного типа модели не делают бросок на попадание. Вместо этого выберите одну вражескую модель, видимую
для стреляющего, и поместите маркер взрыва так, чтобы его отверстие находилось точно над подставкой выбранной цели (см. рис.) или над корпусом, если это техника. Отверстие в центре маркера должно находиться в пределах максимальной дальности оружия. Нельзя помещать маркер взрыва так, что он хотя бы немного задевал подставку или корпус любой дружественной модели.
Ввиду большого пространства, покрываемого при взрыве, предполагается, что по цели очень трудно совсем не попасть. Тем не менее выстрел
может попасть не точно туда, куда планировалось. Сделайте бросок на смещение (с. 12) маркера взрыва и вычтите значение BS стреляющего из расстояния, на которое сместился выстрел (если произошло смещение). Это расстояние можно свести до 0". Учтите, что вполне возможно и приемлемо, если выстрел сместится за пределы максимальной или минимальной дальности и линии прямой видимости. Так отображается рикошет, взрыв ракеты в укрытии и прочие случайные события. В таких случаях попадания вычисляются как обычно, и выстрел может попасть
и ранить боевые единицы, находящиеся вне дальности и линии прямой видимости (или даже ваши собственные боевые единицы или модели, вовлечённые в бой). Если выстрел смещается так, что отверстие в середине маркера оказывается за пределами края стола, выстрел просто уходит в никуда.
Как только окончательная позиция маркера взрыва будет определена, посмотрите на него сверху – каждая боевая единица получает по одному попаданию за каждую модель, которая полностью или частично находится под маркером взрыва, даже если сама модель не видна стреляющему.
...
рудия с правилом «большой взрыв» используют маркер взрыва с диаметром 5", а в остальном подчиняются всем правилам оружия взрывного типа.
Огромный взрыв
...
• Сила и AP любого попадания зависят от зоны, в которой находится цель. Первое значение показателей силы и AP применяется для внутренней зоны, второе – для средней, а третье – для внешней. Всегда используйте наилучшие показатели силы и AP, если модель занимает место в двух или более зонах. Если
у боевой единицы есть модели в разных зонах, высчитывайте попадания для каждой зоны отдельно. Учтите, у каждой зоны, следовательно, будет собственный фонд ранений.
...
'
    ...
    textEng : 'Large Blast weapons use the 5" blast marker, but otherwise obey all the rules for Blast
weapons.',
    ...
    visibleName: 'Blind',
    textEng : 'A unit containing at least one model with this special rule is a Psyker unit – if no Mastery
...
• If this unit suffers Perils of the Warp, or is hit by an attack that specifically targets
...
power, all other models with this special rule in their unit also gain or lose that power',
    nameRus : 'Ослепление',    
    textRus : 'Любая боевая единица, получившая попадание от одной или более моделей или единиц оружия с этим специальным правилом, должна пройти тест на инициативу в конце текущей фазы. Если тест успешно пройден, все хорошо – кто-то выкрикивает предупреждение, и воины успевают отвернуться. Если тест на инициативу провален, то показатели навыков стрельбы и ближнего боя у всех моделей боевой единицы снижаются до
1 пункта до конца их следующего хода.
В том случае, если атакующая боевая единица попадает в себя, то считается, что она была к этому подготовлена, и потому автоматически проходит тест. Любая модель без характеристики инициативы (например, строения или техника,
не считая шагоходов) не подвержена воздействию данного спецправила.'
}, {
...
• При проявлении психосилы данная боевая единица отмеряет расстояние и линию прямой видимости от любой одной своей модели (а
также использует её перечень характеристик при необходимости) со специальным правилом «братство псайкеров/чернокнижников» (на усмотрение управляющего игрока).
• Если боевая единица подвергается опасностям варпа или получает попадание от атаки, направляемой специально на псайкеров, попадания распределяются случайным образом среди моделей со специальным правилом «братство псайкеров/ чернокнижников». Если модель
с этим спецправилом получает
или теряет психосилу, все прочие модели с этим спецправилом в их боевой единице тоже получают или теряют данную силу.'
}, {
...
или теряет психосилу, все прочие модели с этим спецправилом в их боевой единице тоже получают или теряют данную силу.'
    ...
    textEng : 'A model that suffers one or more unsaved Wounds from a weapon with this special rule
is reduced to Initiative 1 until the end of the following Assault phase.',
    ...
    textEng : 'If a unit contains at least one model with this special rule, and that unit is charged, every
...
has no effect.',
    ...
    textEng : 'A unit that contains at least one model with this special rule rolls an extra dice when
making Run moves, and uses the highest result rolled. In addition, a unit that contains at
least one model with this special rule adds D3 to its Sweeping Advance totals (roll each
time).',
    nameRus : 'Крестоносец',    
    ...
    visibleName: 'Daemon',
    textEng : 'Models with the Daemon special rule have a 5+ invulnerable save, and also have the Fear
special rule.',
    nameRus : 'Крестоносец',    
    ...
    textEng : 'If a weapon has a D instead of a Strength value in its profile, it is a Destroyer weapon. To
...
Cover saves and invulnerable saves can be taken against hits from a Destroyer weapon as
...
2-5
Vehicle or Building - Solid Hit: The model suffers a penetrating hit that
causes it to lose D3 Hull Points instead of 1.
Non-vehicle - Seriously Wounded: The model suffers a hit that wounds
automatically and causes it to lose D3 Wounds instead of 1.
6
Vehicle or Building - Devastating Hit: The model suffers a penetrating hit
that causes it to lose D6+6 Hull Points instead of 1. No saves of any kind are
allowed against this hit.
Non-vehicle - Deathblow: The model suffers a hit that wounds
automatically and causes it to lose D6+6 Wounds instead of 1. No saves of any
kind are allowed against this hit.

',
    nameRus : 'Деструктивные орудия',    
    textRus : 'Если в профиле орудия вместо
показателя силы стоит «D» (Destroyer), значит, оно имеет специальное правило «деструктивное орудие». Чтобы отыграть атаку деструктивного орудия, сделайте бросок на попадание, как при обычной атаке. При попадании сделайте бросок
по таблице наверху вместо броска на ранение или на пробитие брони. У большинства видов деструктивных орудий AP1 или AP2, так что спас-броски за броню, как правило, не дозволяются. Спас-броски за укрытие и непробиваемые спас-броски применяются как обычно, если только не выпадают результаты «опустошительное попадание» или «смертельный удар». Чтобы определить, получило ли попадание деструктивного орудия спецправило «мгновенная смерть», допускайте, что оно имеет
силу 10. Если деструктивное орудие при попадании наносит несколько ранений или снимает несколько пунктов брони,
не распределяйте их на другие модели боевой единицы (любые излишки просто сгорают).
...
6
Опустошительное попадание:
модель получает пробивающее попадание, которое снимает D6+6 пунктов брони вместо одного. Никакого рода спас-броски не дозволяются.
Смертельный удар: модель
получает попадание, которое наносит автоматическое ранение и забирает D6+6 ран вместо одной. Никакого рода спас- броски не дозволяются.
￼
'
}, {
    specialRuleName: 'DeepStrike',
    visibleName: 'Deep strike',
    textEng : 'In order for a unit to be able to Deep Strike, all models in the unit must have the Deep
...
Deep Striking onto a crowded battlefield can be dangerous, as one may miss the intended
objective or even materialise inside solid rock! If any of the models in a Deep Striking
...
Strike Mishap table and apply the results. If the unfortunate unit is also a Transport, the
Deep Strike Mishap result applies to both the unit and anything embarked within it.

Deep Strike Mishap Table

D6 - Effect

1 - Terrible Accident! Teleporting units are lost in the Warp, deep striking
jump units are shot down, or some other suitably dramatic event occurs.
The entire unit is destroyed!

2-3 - Misplaced. The coordinates were inaccurate or the enemy has jammed
your instruments.
Your opponent may deploy the unit anywhere on the table (excluding
impassable terrain, but including difficult terrain, which of course counts as
dangerous for Deep Striking units), in a valid Deep Strike formation, but
without rolling for scatter. Units embarked on a misplaced Transport can
disembark during their Movement phase as normal.

4-6 - Delayed. Because of mechanical failure or enemy action, the
...
Для того чтобы боевая единица смогла провести глубокий удар, все её модели должны обладать данным специальным правилом, и к тому же боевая единица должна начинать игру в резерве. Убирая боевую единицу в резерв, вы должны предупредить своего соперника, что она прибудет посредством глубокого удара (иногда говорят «прибудет из глубокого резерва»).\n\nНекоторые боевые единицы должны прибывать только посредством глубокого удара. Они всегда начинают игру в резерве и всегда прибывают только посредством глубокого удара. При подсчете количества боевых единиц, которые можно убрать в резерв, не учитываются боевые единицы (вместе с любыми погруженными на/в них моделями), способные прибывать только посредством глубокого удара. Вдобавок, боевая единица, которая должна прибывать посредствам глубокого удара (например, десантная капсула), должна так делать, даже если вы играете в специальную миссию, где не используется специальное правило «резервы». Разумеется, все миссии Вечной войны, представленные дальше, используют резервы, поэтому чаще всего вы можете не беспокоиться об этой особенности.\n\nПрибытие посредством глубокого удара\n\nДелайте броски за прибытие всех боевых единиц, использующих глубокий удар, как установлено в правилах на резервы, и затем расставляйте их следующим образом:\n\nВо-первых, поместите одну модель боевой единицы в любое место на столе, куда бы вы хотели, чтобы она прибыла, и бросьте кубик смещения для определения окончательной позиции модели. Если техника смещается, прибывая посредством глубокого удара, не меняйте её направление – она должна продолжать смотреть в том же направлении, что и до броска на смещение.\nВо-вторых, остальные модели боевой единицы расставляются вокруг первой. Модели необходимо располагать вокруг первой в базовом контакте с ней. По завершении заполнения первого круга модели размещаются в последующих внешних кругах так, чтобы они соприкасались с моделями внутреннего круга. Каждый круг должен включать как можно больше моделей.\nПри расстановке прибывших посредством глубокого удара моделей труднопроходимый ландшафт считается опасным.\nВ фазе движения (во время которой они и прибывают) боевые единицы, использующие глубокий удар, не могут больше передвигаться, кроме как при высадке из транспортной техники, прибывшей посредством глубокого удара, если они в ней находились. Боевые единицы, появляющиеся посредством глубоком удара в руинах, расставляются на первом этаже. Прибывающие посредством глубокого удара боевые единицы считают неразрушенные строения (за исключением их парапетов) непроходимым ландшафтом.\n\nВ фазе стрельбы текущего хода данные боевые единицы могут стрелять (или бежать) как обычно и, разумеется, считаются передвигавшимися в прошлой фазе движения. Техника (за исключением шагоходов) считается двигавшейся на крейсерской скорости (даже неподвижная техника). Это влияет на количество орудий, из которых они могут стрелять с целым показателем навыка стрельбы (см. стр. 71). \n\nВ фазе наступления текущего хода, однако, данные боевые единицы не могут нападать. Это также относится к боевым единицам, которые высадились из транспортов, прибывших посредством глубокого удара в этом ходе, в том числе и десантно-штурмовых. \n\nНесчастные случаи при глубоком ударе\n\nСовершение глубокого удара посреди живых толп на поле боя может быть опасно, так как можно появиться за мили от намеченного места или даже материализоваться внутри твёрдой скалы! Если какую-либо модель прибывающей по- средством глубокого удара боевой единицы нельзя поста- вить, потому что хотя бы одна модель приземлится частич- но или полностью за столом, на непроходимом ландшафте, над дружественной или вражеской моделью, а также в пре- делах 1" от вражеской модели, значит что-то пошло не так. Управляющий игрок должен кинуть D6 за несчастный слу- чай при глубоком ударе и следовать правилам по выпавшему результату. Если неудачливая боевая единица оказывается ещё и транспортом, тогда результат несчастного случая при глубоком ударе относится как к самой боевой единице, так и ко всем погруженным в неё.'
    ...
    textEng : 'If a model with this special rule suffers an unsaved Wound from an attack that inflicts
...
Wounds to 0.',
    ...
    visibleName: 'Fear',
    textEng : 'At the start of each Fight sub-phase, a unit in base contact with one or more enemy
...
models that cause Fear.',
    ...
    textEng : 'Units containing one or more models with the Fearless special rule automatically pass
...
immediately cancelled.',
    nameRus : 'Бесстрашие',    
    textRus : 'Боевые единицы, содержащие одну
или более моделей со специальным правилом «бесстрашие», автоматически проходят тесты на подавление огнём, устрашение, перегруппировку и боевой дух, но не могут залегать и добровольно проваливать проверку на боевой дух
по правилу «наше оружие бесполезно» (с. 53). Если боевая единица залегла,
а уже потом получила спецправило «бесстрашие», залегание немедленно прекращается.'
}, {
    specialRuleName: 'FeelNoPain',
    visibleName: 'Feel no pain',
    textEng : 'When a model with this special rule suffers an unsaved Wound, it can make a special Feel
...
Если боевая единица имеет специальное правило «не ведая боли» с заключённым в скобки числом – например, «не ведая боли (6+)» – тогда число в скобках показывает необходимый результат D6 для отмены ранения.'
    ...
    visibleName: 'Fleet',
    textEng : 'A unit composed entirely of models with this special rule can re-roll one or more of the
dice when determining Run moves and charge ranges (such as a single D6 from a charge
...
In either case, this special rule has no effect against vehicles or building',
    nameRus : 'Губитель плоти',    
    textRus : 'Если модель имеет это специальное правило или атакует рукопашным оружием с этим специальным правилом, она всегда наносит ранение на 2+ в ближнем бою.
Аналогичным образом, если модель производит стрелковую атаку оружием с этим специальным правилом, она всегда наносит ранение на 2+.
В любом случае данное специальное правило не действует на технику или строения.'
}, {
    specialRuleName: 'Force',
    visibleName: 'Force',
    textEng : 'Any Psyker that has one or more weapons with this special rule knows the Force psychic
power in addition to any other powers they know:
...
',
    ...
    Ментальная Мощь
1 варп-заряд
Псайкер направляет ментальные силы в пси- схемы своего психосилового оружия, преобразуя его из обычного в такое, которое способно разрывать реальность.
Ментальная мощь считается благословением, направляемым на самого псайкера и его боевую единицу. Всё их оружие со специальным правилом «психосиловое» получает спецправило «мгновенная смерть» до начала вашей следующей пси-фазы.

'
}, {
    specialRuleName: 'FuriousCharge',
    visibleName: 'Furious charge',
    textEng : 'In a turn in which a model with this special rule charges into combat, it adds +1 to its
...
disordered charge that turn receives no benefit from Furious Charge.',
    ...
    textEng : 'When firing a weapon that Gets Hot, roll To Hit as normal. For each To Hit roll of 1, the
...
￼'
    ...
    textEng : 'The roll needed To Wound when firing a weapon with this special rule is always equal to
...
and loses a Hull Point. These weapons have no effect on buildings.',
    nameRus : 'Гравитонное',    
    textRus : 'При стрельбе из оружия с этим специальным правилом значение броска, необходимого для нанесения ранения, всегда равно показателю спас-броска за броню цели (минимум нужно выкинуть 6+). Например, при отыгрыше попадания против модели со спас-броском за броню на 3+ вам нужно выбросить 3 или больше для нанесения ранения. При отыгрыше попадания против техники кидайте D6
за каждое попадание вместо проведения привычного броска на пробитие брони. На 1-5 ничего не происходит, а при выпадении 6 техника обездвиживается и теряет пункт брони. Данное оружие не действует на строения.'
}, {
    specialRuleName: 'HammerOfWrath',
    visibleName: 'Hammer ofWrath',
    textEng : 'If a model with this special rule ends its charge move in base or hull contact with an
enemy model, it makes one additional Attack that hits automatically and is resolved at the
model’s unmodified Strength with AP-. This Attack does not benefit from any of the
...
resolved against the building or vehicle, not the occupants or the rider.',
    ...
    textRus : 'Если модель с этим правилом завершает свой наступательный манёвр в
базовом или корпусном контакте с вражеской моделью, она делает одну дополнительную атаку, которая наносит автоматическое попадание, отыгрываемое с немодифицированной силой модели
и AP-. Эта атака не получает бонусов
от каких-либо специальных правил модели (таких как «яростное нападение», «раздирающее» и т.д.). Данная атака отыгрывается во время подфазы
схватки на 10 этапе инициативы, но не предоставляет модели дополнительного манёвра ввязывания в бой.
Если модель с этим специальным правилом нападает на строение или технику, попадание отыгрывается против показателя брони той стороны, с которой соприкасается нападающая модель. Если модель с этим специальным правилом нападает на строение или технику, которая относится к транспорту или колеснице, попадания по правилу «молот ярости» отыгрываются по строению
или технике, а не по занявшим строение моделям или наезднику.'
}, {
    specialRuleName: 'Hatred',
    visibleName: 'Hatred',
    textEng : 'This rule is often presented as Hatred (X) where X identifies a specific type of foe. If the
...
close combat.',
    ...
    visibleName: 'Haywire',
    textEng : 'When a weapon with this special rule hits a vehicle, roll a D6 to determine the effect
...
6 - Penetrating hit',
    ...
    textEng : 'A unit that contains at least one model with this special rule that is locked in combat can
...
inches, is sufficient to allow the entire unit to move over 1" away from all of the enemy
units they are locked in combat with, the unit breaks away from combat and must
...
A Hit & Run move is not slowed by difficult terrain, but takes Dangerous Terrain tests as
...
disengage, roll-off to determine who goes first and then alternate disengaging them. If the
...
Если тест успешно пройден, выберите направление и затем киньте 3D6".
При условии, что выброшенного расстояния хватает, чтобы позволить вашей боевой единице уйти более чем
на 1" от всех вражеских боевых единиц, боевая единица вырывается из боя и немедленно двигается в указанном направлении на равное результату броска количество дюймов, не обращая внимания на вражеские боевые единицы, с которыми была связана боем. Броски на преследование не дозволяются. Вражеские боевые единицы, уже
не связанные боем, немедленно объединяются на D6".
При движении по правилу «ударил- отступил» боевая единица не замедляется на труднопроходимом ландшафте, но проходит тесты на преодоление опасного ландшафта как обычно. Движение по данному правилу нельзя использовать, чтобы вступить в базовый или корпусный контакт с вражескими боевыми единицами, ваши модели должны остановиться в 1" от них. Если у обеих сторон есть боевые единицы, которые хотят выйти из боя, то методом кубовки определите, кто ходит первым, и затем поочерёдно выведите их. Последняя боевая единица, которая больше не связана боем, вместо этого объединяется.'
    ...
    textEng : 'Cover saves cannot be taken against Wounds, glancing hits or penetrating hits caused by
weapons with the Ignores Cover special rule.',
    ...
    textEng : 'Independent Characters can join other units. They cannot, however, join units that
...
Heroic Morale
A unit that contains one or more Independent Characters does not need a double 1 to
Regroup if reduced to below 25% of its starting numbers, but instead tests as if it had at
least 25% remaining.
Special Rules
...
An Independent Character without the Infiltrate special rule cannot join a unit of
...
For example, Crimson Fists Tactical Squad Hemanez is suffering the effects of the Blind
special rule. If Captain Cruiz joins the unit, he does not suffer the results of the Blind
special rule as he was not there when it happened!',
...
Чтобы присоединиться к боевой единице, независимому персонажу нужно просто приблизиться так, чтобы он находился на расстоянии 2" в боевом построении с дружественной боевой единицей в конце своей фазы движения. Если независимый персонаж находится на расстоянии 2" более чем с одной боевой единицей в конце своей фазы движения, игрок должен объявить, к какой боевой единице он присоединяется. Если независимый персонаж не собирается (или не может) присоединиться к боевой единице, он должен (при возможности) оставаться на расстоянии более
2" от неё в конце фазы движения. Это необходимо, чтобы определить, присоединился ли он к боевой единице или нет. Учтите, что после того как независимый персонаж присоединится к боевой единице, данная боевая единица не может больше передвигаться в этой фазе движения.
...
Независимый персонаж не может оставить боевую единицу, пока он
или боевая единица находятся в резерве, связаны боем, отступают или залегли. Он не может присоединиться к боевой единице, находящейся в резерве, связанной боем или отступающей. Если независимый персонаж присоединился к боевой единице, и все остальные модели этой боевой единицы погибли, он снова становится отдельной боевой единицей из одной модели в начале последующей фазы.
...
Иногда боевая единица, к которой присоединяется независимый персонаж, испытывает на себе полезное или вредное воздействие,
как, например, получаемое при специальном правиле «ослепление». Если персонаж оставляет боевую единицу, то и он, и боевая единица продолжают испытывать на себе то или иное воздействие, поэтому вам необходимо отдельно помечать персонажа.
...
Однако если персонаж присоединяется к боевой единице после
того, как боевая единица стала жертвой длящегося воздействия (или присоединяется после того, как сам стал жертвой длящегося воздействия), преимущества и штрафы от этого воздействия ему не распределяются.
К примеру, тактическое отделение Хеманеза из ордена Багровых Кулаков испытывает на себе воздействие специального правила «ослепление». Если капитан Круз присоединяется к боевой единице, то на него не действуют результаты специального правила «ослепление», так как его не было там, когда это случилось!'
    ...
    textEng : 'Units that contain at least one model with this special rule are deployed last, after all
other units (friend and foe) have been deployed. If both sides have Infiltrators, the
players roll-off and the winner decides who goes first, then alternate deploying these
units.
...
Having Infiltrate also confers the Outflank special rule to units of Infiltrators that are
kept as Reserves.
Outflank
During deployment, players can declare that any unit that contains at least one model
with this special rule is attempting to Outflank the enemy.
When this unit arrives from Reserves, but not Ongoing Reserve, the controlling player
rolls a D6: on a 1-2, the unit comes in from the table edge to the left of their controlling
player’s own table edge; on a 3-4, they come on from the right; on a 5-6, the player can
choose left or right. Models move onto the table as described for other Reserves.
If a unit with Infiltrate deploys inside a Dedicated Transport, they may Outflank along
with their Transport.
Infiltrate and Scout
If a unit has both the Infiltrate and Scout special rule, that unit can deploy as per the
Infiltrate special rule and then redeploy as per the Scout special rule.',
...
￼'
    ...
    textEng : 'If a model suffers an unsaved Wound from an attack with this special rule, it is reduced to
0 Wounds and is removed as a casualty.',
    ...
    textEng : 'At the end of the enemy Movement phase, a weapon with the Interceptor special rule can
...
shoot a different weapon if it has one.',
    ...
    textRus : 'В конце фазы движения противника оружие со специальным правилом «перехватывающее» может выстрелить по любой одной боевой единице,
которая прибыла из резерва и находится на линии прямой видимости и в
пределах досягаемости оружия. При использовании этого правила оружие не может стрелять в следующем ходе, но стреляющая модель может вести огонь из другого оружия, если оно есть.'
}, {
    specialRuleName: 'ItWillNotDie',
    visibleName: 'It will not die',
    textEng : 'At the end of each of your turns, roll a D6 for each of your models with this special rule
...
Point, lost earlier in the game.',
    ...
    textRus : 'В конце каждого дружественного хода бросайте D6 за каждую модель с этим специальным правилом, у которой осталось меньше ран или пунктов
брони корпуса, чем было изначально,
но она не была удалена как убитая или уничтоженная. При выпадении 5+ модель восстанавливает одну рану или пункт брони корпуса, ранее потерянные в битве.'
}, {
    ...
    visibleName: 'Jink',
    textEng : 'When a unit with any models with the Jink special rule is selected as a target for a
...
Snap Shots until the end of their next turn.',
    nameRus : 'Уклонение',
    textRus : 'Если целью стрелковой атаки становится ваша боевая единица
с любыми моделями, имеющими специальное правило «уклонение», вы можете объявить, что она уклоняется. Это решение необходимо принять
до совершения любых бросков на попадание. Если боевая единица уклоняется, все её модели с этим специальным правилом получают спас- бросок за укрытие на 4+ до начала своей следующей фазы движения, но могут стрелять только навскидку до конца своего следующего хода.'
}, {
    specialRuleName: 'Lance',
    visibleName: 'Lance',
    textEng : 'Weapons with the Lance special rule count vehicle Armour Values that are higher than 12
as 12.',
    ...
    textEng : 'Weapons with the Master-crafted special rule allow the bearer to re-roll one failed roll To
Hit per turn with that weapon.',
    ...
    textEng : 'When a building with this special rule suffers a penetrating hit, there is a -1 modifier to
the roll on the Building Damage table.',
    ...
    visibleName: 'Melta',
    textEng : 'Ranged weapons with this special rule roll an additional D6 when rolling to penetrate a
...
If a weapon has both the Melta and Blast special rules, measure the distance to the centre
of the blast marker after it has scattered. If this is half the weapon’s range or less, all hits
caused by the blast marker roll an addition D6 when rolling to penetrate a vehicle’s
armour. If the centre of the blast marker is more than half the weapon’s maximum range
away after scatter, roll to penetrate as normal.
...
Если у оружия есть правила «мелта» и «взрыв» (с. 158), отмеряйте расстояние до центра маркера взрыва после того, как он сместится. Если расстояние
равно половине дальности действия оружия или меньше, стреляющий
кидает дополнительный D6 при броске на пробитие брони техники за каждое попадание, нанесённое маркером взрыва. Если после смещения центр маркера взрыва находится дальше половины дальности действия оружия, делайте бросок на пробитие как обычно.
См. правила техники (с. 72) для получения подробной информации о пробитии брони.'
    ...
    textEng : 'A model with this special rule re-rolls failed To Hit rolls when shooting any weapon that
...
Blast special rules, that shot will instead scatter D6" rather than 2D6".',
    ...
    textRus : 'Модель с этим специальным правилом перекидывает проваленные броски
на попадание при стрельбе из любого одноразового оружия.
Если модель с этим специальным правилом стреляет из оружия, у которого есть спецправила «одноразовое» и «взрыв», выстрел смещается на D6" вместо 2D6".'
    ...
    textEng : 'A unit that contains at least one model with this special rule re-rolls all failed To Wound
rolls against Monstrous Creatures.',
    ...
    textEng : 'A unit that contains at least one model with this special rule rolls an extra D6 when
rolling to move through difficult terrain and is not slowed by charging through difficult
...
automatically passes Dangerous Terrain tests.',
    ...
    textEng : 'A unit that contains at least one model with this special rule ignores the effects of Night
Fighting.',
    nameRus : 'Ночной бой',    
    ...
    textEng : 'During deployment, players can declare that any unit that contains at least one model
with this special rule is attempting to Outflank the enemy. This means they are making a
wide sweeping move to get behind enemy lines or come at the foe from an unexpected
direction.
...
Reserves. If such a unit deploys inside a Dedicated Transport, they may Outflank along
with their Transport.',
    nameRus : 'Фланговый обход',    
    textRus : 'Во время расстановки игроки могут объявить, что любая боевая единица, содержащая хотя бы одну модель с этим специальным правилом, пытается обойти врага с фланга. Это означает, что она совершают крутой и стремительный маневр, чтобы зайти за вражеские
линии или подойти к противнику с неожиданного направления.
Когда боевая единица, совершающая фланговый обход, прибывает из
резерва (но не действующего резерва) управляющий ею игрок кидает D6. На 1-2 боевая единица выходит с левого края стола относительно края стола управляющего ею игрока; на 3-4 она выходит справа; на 5-6 игрок может выбрать, с какой стороны она выйдет
– справа или слева. Модели выходят на стол, как описано для всех прибывающих из резерва. Если такая боевая единица размещена в приданном транспорте, то она может воспользоваться правилом «фланговый обход» вместе со своим транспортом.'
}, {
    specialRuleName: 'OneUseOnly',
    visibleName: 'One Use Only',
    textEng : 'A weapon or ability with this special rule can only be used once during the course of a
battle.',
    nameRus : 'Одноразовое',    
    ...
    visibleName: 'Pinning',
    textEng : 'If a non-vehicle unit suffers one or more unsaved Wounds from a weapon with the
Pinning special rule, it must take a Leadership test once the firing unit has finished its
...
already taken its saves, Going to Ground does not protect it against the fire of the Pinning
...
wish.',
    ...
    textEng : 'If a model has the Poisoned special rule, or is attacking with a Melee weapon that has the
Poisoned special rule, it always wounds on a fixed number (generally shown in brackets),
...
rule, it always wounds on a fixed number (generally shown in brackets), unless a lower
...
Poisoned special rule has no effect against vehicles.',
    nameRus : 'Отравленное',    
    ...
    textEng : 'In a turn in which the vehicle neither moves Flat Out nor uses smoke launchers, the
vehicle can fire one more weapon at its full Ballistic Skill than normally permitted. In
addition, this weapon can be fired at a different target unit to any other weapons, subject
to the normal rules for shooting.',
    ...
    textEng : 'This rule is often presented as Preferred Enemy (X) where X identifies a specific type of
...
shooting and close combat attacks.',
    ...
    textEng : 'If a model with this special rule rolls a 6 To Hit with a shooting weapon, that shot is a
‘Precision Shot’.
...
Учтите, что точных выстрелов никогда не может быть при стрельбе навскидку, а также при стрельбе из оружия, выстрелы которого смещаются или для них не надо делать броска на попадание.'
    ...
    textEng : 'If a model with this special rule rolls a 6 To Hit with a Melee weapon, that hit is a
‘Precision Strike’.
Wounds from Precision Strikes are allocated against an engaged model (or models) of
...
}, 
{
    ...
    textEng : 'A vehicle with this special rule is a Psyker. This rule is typically presented with a Mastery
...
manifesting psychic powers, with the following clarification: the vehicle is considered to
have a Leadership characteristic of 10, should this be needed in order to resolve any
psychic power or Perils of the Warp.',
    nameRus : 'Пси-пилот',    
    textRus : 'Техника с этим специальным правилом является псайкерской. Вместе с данным правилом в скобках обычно указывается и уровень мастерства – если УМ не указан, то считается, что у техники УМ 1. Правила генерирования и проявления психосил можно найти на странице
23. Боевая единица следует обычным правилам генерирования и проявления психосил, но с некоторыми уточнениями: считается, что лидерство техники равно 10, если требуется отыграть какие-либо психосилы или опасности варпа.'
}, 
{
    specialRuleName: 'Psyker',
    visibleName: 'Psyker',
    textEng : 'A model with this special rule is a Psyker. This rule is typically presented with a Mastery
...
}, 
{
    ...
    visibleName: 'Rage',
    textEng : 'In a turn in which a model with this special rule charges into combat, it gains +2 Attacks
for charging, rather than +1. A model that has made a disordered charge that turn receives
no benefit from Rage.',
    nameRus : 'Ярость',    
    textRus : 'Если модель с этим правилом нападает на противника, в текущем ходе она получает +2 атаки за нападение вместо +1. Модель, совершившая беспорядочное нападение
в текущем ходе, не получает никаких преимуществ от «ярости» (см. с. 54).'
}, 
{
    specialRuleName: 'Rampage',
    visibleName: 'Rampage',
    textEng : 'IAt the start of any Fight sub-phase, models with the Rampage special rule gain +D3
attacks if the combat they are in contains more enemy models than friendly models –
...
benefit from Rampage.',
    nameRus : 'Неистовство',    
    textRus : 'В начале любой подфазы схватки модели со специальным правилом «неистовство» получают +D3 атак, если они участвуют
в бою, где находится больше вражеских, чем дружественных моделей – считаются все модели каждой связанной этим боем боевой единицы, а не только вовлечённые в этот бой модели. Сделайте всего один бросок, чтобы вычислить бонусные атаки в текущей фазе сразу у всех
неистовствующих моделей, вовлечённых в этот бой. Модель, совершившая беспорядочное нападение в текущем ходу, не получает никаких преимуществ от «неистовства» (см. с. 54).'
}, 
{
    ...
    textRus : 'Безостановочные модели могут
стрелять из тяжёлых, залповых или артиллерийских орудий, как если
бы оставались неподвижными, даже если на самом деле двигались в предшествующей фазе движения.
Кроме того, им разрешено нападать в том же ходе, в каком они стреляли из тяжёлых, залповых, скорострельных или артиллерийских орудий.'
}, 
{
    ...
    visibleName: 'Rending',
    textEng : 'If a model has the Rending special rule, or is attacking with a Melee weapon that has the
...
instead resolved using the model/weapon’s AP value',
    nameRus : 'Раздирающее',    
    textRus : 'Если модель имеет специальное правило «раздирающее» или атакует рукопашным оружием с этим специальным правилом, то есть шанс, что атака в ближнем бою причинит критический урон. При каждом выпадении 6 при броске на ранение
цель автоматически получает ранение вне зависимости от её стойкости. Такие ранения отыгрываются с AP2.
...
В обоих случаях каждое выпадение 6 при броске на пробитие брони техники позволяет кинуть ещё D3, результат которого прибавляется к сумме. Эти попадания отыгрываются не с AP2, а с показателем AP модели/оружия.'
    ...
    textEng : 'Models disembarking from Access Points on a building can charge on the turn they do so,
even on a turn the building was destroyed.',
    ...
    visibleName: 'Scout',
    textEng : 'After both sides have deployed (including Infiltrators), but before the first
player begins his first turn, a unit containing at least one model with this
...
but must remain more than 12" away from any enemy unit. A unit that makes
a Scout redeployment cannot charge in the first game turn. A unit cannot
embark or disembark as part of a Scout redeployment.
If both sides have Scouts, roll-off; the winner decides who redeploys first. Then
alternate redeploying Scout units.
If a unit with this special rule is deployed inside a Dedicated Transport, it
confers the Scout special rule to the Transport (though a disembarkation
cannot be performed as part of the redeployment). Note that a Transport with
this special rule does not lose it if a unit without this special rule is embarked
upon it. Having Scout also confers the Outflank special rule to units of Scouts
that are kept as Reserves.
Outflank
During deployment, players can declare that any unit that contains at least one
model with this special rule is attempting to Outflank the enemy. When this
unit arrives from Reserves, but not Ongoing Reserve, the controlling player
rolls a D6: on a 1-2, the unit comes in from the table edge to the left of their
controlling player’s own table edge; on a 3-4, they come on from the right; on a
5-6, the player can choose left or right. Models move onto the table as
described for other Reserves.
If a unit with Scouts is deployed inside a Dedicated Transport, they may
Outflank along with their Transport.
Iinfiltrate and Scout
If a unit has both the Infiltrate and Scout special rule, that unit can deploy as
per the Infiltrate special rule and then redeploy as per the Scout special rule.',
    nameRus : 'разведчик',    
    textRus : 'После того как обе стороны расставят свои армии (в том числе и лазутчиков), но перед
тем, как первый игрок начнёт свой первый
ход, боевая единица, содержащая хотя бы одну модель с этим специальным правилом, может по своему желанию сделать перестановку. Если боевая единица относится к пехоте, артиллерии, шагоходам или монструозным созданиям,
то каждую модель можно переставить куда угодно в пределах 6" от её текущей позиции. Если она относится к другим классам боевых единиц, то каждую модель можно переставить в пределах 12" от её текущей позиции. Во время этой перестановки разведчики могут выйти
за пределы зоны расстановки управляющего игрока, но должны оставаться на расстоянии более 12" от любой вражеской боевой единицы. Боевая единица, которая совершает разведывательную перестановку, не может нападать в первом игровом ходе. Боевая единица не может использовать свой манёвр разведывательной перестановки для погрузки или высадки из техники или входа или выхода из строения.
...
Если боевая единица с этим специальным правилом размещается внутри приданного транспорта, то транспорт получает специальное правило «разведчик» (хотя во время перестановки нельзя производить высадку). Учтите, что транспорт с этим специальным правилом не теряет его, если в него погружается боевая единица без этого специального
правила. Разведчики со специальным правилом «проникновение», которые находятся в резерве (см. с. 135), получают ещё и специальное правило «фланговый обход».
Фланговый обход
...
Если у боевой единицы есть и спецправило «проникновение», и спецправило «разведчик», данная боевая единица может расставиться по спецправилу «проникновение», а затем заново расставиться по спецправилу «разведчик».'
    ...
    textEng : 'A building with this special rule can use automated fire against enemy units, even if it is
...
rule, even if it is unoccupied.',
    ...
    textEng : '',
    nameRus : 'Кромсающее',    
    textRus : 'Если модель имеет специальное правило «кромсающее» или атакует рукопашным оружием с этим специальным правилом, она перекидывает проваленные броски на ранение в ближнем бою.
Аналогичным образом, если модель совершает стрелковую атаку оружием с этим специальным правилом, она перекидывает проваленные броски на ранение.'
}, {
    specialRuleName: 'Shrouded',
    visibleName: 'Shrouded',
    textEng : 'A unit that contains at least one model with this special rule counts its cover save as being
...
maximum of a 2+ cover save).',
    ...
    textRus : 'У боевой единицы, содержащей хотя
бы одну модель с этим специальным правилом, спас-броски за укрытие считаются на 2 пункта выше, чем обычно. Учтите, это означает, что модель со специальным правилом «скрытый пеленой», всегда имеет спас-бросок за укрытие как минимум на 5+, даже если она находится на открытой местности.
Бонусы для спас-бросков за укрытие
по специальным правилам «скрытый пеленой» и «проникновение» суммируются (максимум возможен спас- бросок за укрытие на 2+).'
}, {
    specialRuleName: 'SkilledRider',
    visibleName: 'Skilled rider',
    textEng : 'A unit that contains at least one model with this special rule automatically passes
Dangerous Terrain tests, and receives +1 to its Jink cover saves (other cover saves are
unaffected).',
    nameRus : 'Опытный наездник',    
    textRus : 'Боевая единица, содержащая хотя
бы одну модель с этим специальным правилом, автоматически проходит тесты на преодоление опасного ландшафта и получает +1 к спас-броскам за укрытие по правилу «уклонение» (другие спас- броски за укрытие остаются неизменны).'
}, {
    specialRuleName: 'Skyfire',
    visibleName: 'Skyfire',
    textEng : 'A model with this special rule, or that is firing a weapon with this special rule, fires using
...
Skimmers, but it can only fire Snap Shots against other targets.',
    ...
    textRus : 'Модель с этим специальным правилом или стреляющая оружием с этим специальным правилом использует
свой обычный навык стрельбы при ведении огня по летательным аппаратам, летающим монструозным созданиям и скиммерам, но по другим целям может вести лишь стрельбу навскидку.'
}, {
    specialRuleName: 'SlowAndPurposefu',
    visibleName: 'Slow and Purposefu',
    textEng : 'A unit that contains at least one model with this special rule cannot Run, Turbo-boost,
move Flat Out, perform Sweeping Advances or fire Overwatch. However, they can shoot
with Heavy, Salvo and Ordnance weapons, counting as stationary even if they moved in
the previous Movement phase. They are also allowed to charge in the same turn they fire
Heavy, Ordnance, Rapid Fire or Salvo weapons.',
    ...
    visibleName: 'Smash',
    textEng : 'All of the close combat attacks, except Hammer of Wrath attacks, of a model with this
...
its armour penetration rolls, but must abide by the second result.',
    ...
    textRus : 'Боевая единица, содержащая хотя
бы одну модель с этим специальным правилом, не может бегать, турбоускоряться, двигаться на полной скорости, выполнять преследование или вести упреждающий огонь. Однако она может стрелять из тяжёлых, залповых
и артиллерийских орудий, будто из неподвижного состояния, даже если
на самом деле она передвигалась в предыдущей фазе движения. Ей также дозволяется нападать в том же ходе,
в котором она стреляла из тяжёлых, артиллерийских, скорострельных или залповых орудий.'
}, {
    specialRuleName: 'Sniper',
    visibleName: 'Sniper',
    textEng : 'If a weapon has the Sniper special rule, or is fired by a model with the Sniper special rule,
...
count as Strength 4.',
    ...
    textRus : 'Если модель имеет специальное правило «снайперское» или стреляет оружием
с этим специальным правилом, каждое выпадение 6 при броске на попадание считается за точный выстрел (с. 169).
Более того, если модель имеет специальное правило «снайперское» или стреляет оружием с этим специальным правилом, её стрелковые атаки всегда наносят ранение на 4+ при броске на ранение, независимо от стойкости жертвы. Вдобавок, любой бросок на ранение, при котором выпадает 6, отыгрывается с AP2.
Считается, что против техники стрелковые атаки оружия или модели с этим специальным правилом имеют силу 4.'
}, {
    specialRuleName: 'SoulBlaze',
    visibleName: 'Soul Blaze',
    textEng : 'If a unit suffers one or more unsaved Wounds from an attack with this special rule, it is
...
less, the flames die out and the unit is no longer ablaze – remove your reminder counter.
...
on it at a time.',
    ...
    textRus : 'Если боевая единица получает одно или более неспасённых ранений при атаке
с этим специальным правилом, она воспламеняется и продолжает гореть
– пометьте её монетой или фишкой в качестве напоминания.
В конце каждого хода бросайте D6 за каждую боевую единицу с фишкой душесжигателя. При выпадении 3 и меньше пламя затухает, и боевая единица более не горит – удалите оставшуюся фишку. При выпадении 4+ боевая единица получает D3 попаданий с силой 4, AP5 и спецправилом «укрытие не спасает». Эти ранения распределяются случайно. Боевая единица не может иметь больше одной фишки душесжигателя в одно и то же время.'
    ...
    textEng : 'A model fighting with this weapon does not receive +1 Attack for fighting with two
...
rule.',
    ...
    textEng : 'When a unit that contains at least one model with this special rule shoots, one model in
the unit can shoot at a different target to the rest of his unit. Once this shooting attack
has been resolved, resolve the shooting attacks made by the rest of the unit. These must
be at a different target, which cannot be a unit forced to disembark as a result of the Split
Firing unit’s initial shooting attack.',
    nameRus : 'Распределение огня',    
    textRus : 'Когда стреляет боевая единица, содержащая хотя бы одну модель с этим специальным правилом, одна модель боевой единицы может выстрелить
по цели, отличной от той, по которой стреляет остальная часть боевой единицы. Как только будет отыграна эта стрелковая атака, отыграйте стрелковые атаки остальной части боевой единицы. Эти атаки должны быть направлены против другой цели, коей к тому же не может быть боевая единица, которая была вынуждена высадиться/выйти из-за стрелковой атаки первой модели, стрелявшей по правилу распределения огня.'
}, {
    specialRuleName: 'Stealth',
    visibleName: 'Stealth',
    textEng : 'A unit that contains at least one model with this special rule counts its cover saves as
...
often presented as Stealth (X) where X indicates a specific type of terrain, such as Stealth
(Woods) or Stealth (Ruins). If this is the case, the unit only gains the benefit whilst it is
in terrain of the specified type.
Cover save bonuses from the Shrouded and Stealth special rules are cumulative (to a
maximum of a 2+ cover save).',
    nameRus : 'Скрытность',    
    textRus : 'У боевой единицы, содержащей хотя
бы одну модель с этим специальным правилом, спас-броски за укрытие считаются на 1 пункт выше, чем обычно. Учтите, это означает, что модель со специальным правилом «скрытность» всегда имеет спас-бросок за укрытие
как минимум на 6+, даже если она находится на открытой местности. Данное правило зачастую представлено в виде «скрытность (X)», где X определяет специфичный вид ландшафта, такой
как, например, скрытность (леса) или скрытность (руины). Боевая единица имеет преимущества только на определённом виде ландшафта, указанном в её профиле.
Бонусы для спас-бросков за укрытие
по специальным правилам «скрытый пеленой» и «скрытность» суммируются (максимум возможен спас-бросок за укрытие на 2+).'
}, {
    specialRuleName: 'StrafingRun',
    visibleName: 'Strafing run',
    textEng : 'When shooting Assault, Heavy, Rapid Fire or Salvo weapons at Artillery, Beasts, Bikes,
...
this vehicle has +1 Ballistic Skill.',
    ...
    textEng : 'When a unit that contains at least one model with this special rule takes Morale checks or
...
бы одну модель с этим специальным правилом, при прохождении тестов на боевой дух или на подавление огнём
не учитывает любые отрицательные модификаторы лидерства. Если
боевая единица одновременно имеет «бесстрашие» и «упорство», то тогда она использует правила для «бесстрашия».'
}, {
    ...
    textRus : 'Сверхзвуковая техника, которая движется на максимальной скорости, должна пройти как минимум 18", как максимум
– 36".'
}, {
    specialRuleName: 'Swarms',
    visibleName: 'Swarms',
    textEng : 'If, when allocating Wounds to a unit with the Swarms special rule, two or more models
...
terrain, but must test for Dangerous Terrain as normal.',
    nameRus : 'Стаи',    
    textRus : 'Если при распределении ранений боевой единицы с правилом «стаи» две или более её моделей находятся от противника на равном удалении, ближайшей к нему всегда считается модель с меньшим числом ран. Если модель с этим правилом получает неспасённое ранение от
оружия взрывного или огнемётного
типа, то каждое неспасённое ранение приравнивается к двум, если только у этого ранения нет правила «мгновенная смерть». Боевая единица, полностью состоящая из моделей с правилом «стаи», не замедляется на труднопроходимом ландшафте, однако должна проходить тест на преодоление опасного ландшафта как обычно.'
}, {
    ...
    textEng : 'Twin-linked weapons don’t get more shots than normal ones, but they give you a better
...
If the scatter dice does not roll a hit, you can choose to re-roll the dice with a Twin-linked
Blast weapon. If you choose to do so, you must re-roll both the 2D6 and the scatter dice.
...
Спаренное оружие огнемётного типа стреляет точно так же, как и одна единица оружия, но вы должны перекидывать проваленные броски на ранение и пробитие брони.'
    ...
    textEng : 'A model attacking with this weapon never receives +1 Attack for fighting with two Melee
weapons.',
    nameRus : 'Двуручное',    
    ...
    visibleName: 'Torrent',
    textEng : 'A weapon with this special rule is treated like any other Template weapon, but when
...
the weapon and the wide end is no closer to the weapon than the narrow end.',
    ...
    textEng : 'Template weapons are indicated by having the word ‘Template’ for their range instead of a
...
unit, resolved at its normal Strength and AP value. Don’t worry about comparing the
length of the template with the distance to the enemy. If the charge is successful, it
doesn’t matter anyway. If the charge failed, we can assume that the enemy ran into range
of the Template weapon and were driven back.
No Escape
If a Template weapon hits a building’s Fire Point or an Open-topped vehicle and there is a
unit embarked inside that building or vehicle, then in addition to any other effects that
unit suffers D6 hits, resolved at the Strength and AP of the weapon. These hits are
Randomly Allocated.
Hellstorm Weapons
Hellstorm weapons have the word ‘Hellstorm’ instead of a range on their weapon profile.
Hellstorm weapons use the hellstorm template, but otherwise obey the rules for other
Template weapons.',
    nameRus : 'Двуручное',    
    textRus : ''
}, {
    ...
    textEng : 'A unit that contains at least one model with this special rule re-rolls failed armour
...
second result must be kept.',
    ...
    textRus : 'Боевая единица, содержащая хотя бы одну модель с этим специальным правилом, перекидывает проваленные броски на пробитие брони техники (и стрелковые атаки, и атаки ближнего боя) и может
по своему желанию перебрасывать скользящие попадания в попытке совершить пробивающее попадание – однако второй результат уже нельзя перекидывать.'
}, {
    ...
    textEng : 'A model attacking with this weapon Piles In and fights at Initiative step 1, unless it is a
Monstrous Creature or a Walker.',
    ...
    textEng : 'A model with this special rule can make an additional pivot on the spot of up to 90° at the
...
'
    ...
    textEng : 'When Swooping or Zooming, this model may savage its prey. At the end of the Movement
...
benefit from any of the model’s special rules, such as Furious Charge, Poisoned, Rending
etc. Against vehicles, these hits are resolved against the target’s side armour.
A model that made a Vector Strike in its Movement phase counts as having already fired
one weapon in its following Shooting phase. However, any additional weapons it fires that
turn can choose a different target to that of the Vector Strike.',
    nameRus : 'Векторный удар',    
    textRus : 'При пикировании или выполнении «свечи» данная модель может наброситься на свою жертву. В конце фазы движения обозначьте одну невовлечённую в бой вражескую боевую единицу, над которой двигалась модель в этом ходе. Эта боевая единица получает
1 попадание (если это ЛА в режиме «свечи» или пикирующий летающий монстр, тогда боевая единица получает D3 попаданий). Если только не указано иное, это попадание отыгрывается с немодифицированной силой модели и AP2, используя случайное распределение. Эти попадания имеют спецправило «укрытие не спасает». Данные попадания не получают никаких бонусов от всяких специальных правил модели, например, яростного нападения, отравленного или раздирающего оружия и т.п. В случае техники эти попадания отыгрываются против её бортовой брони.
Если модель совершила векторный удар в своей фазе движения, то считается,
что она уже стреляла из одного орудия в последующей своей фазе стрельбы. Тем не менее любые дополнительные орудия, из которых она стреляет в этом ходе, могут свободно вести огонь по другим целям, отличным от той, против которой был проведён векторный удар.'
},{
    specialRuleName: 'Vortex',
    visibleName: 'Vortex',
    textEng : 'A weapon with this special rule is a Destroyer weapon and uses a blast marker of some
...
coming from the centre of the marker, in the same manner as a Barrage weapon.
The marker for a Vortex weapon is not removed from play after damage has been
...
arrow if you roll a Hit!). If a double is rolled, the marker is removed from play instead.
Any unit under the marker’s new location is hit. Apply damage as described above.',
    nameRus : 'Вихревое',    
    textRus : 'Оружие с этим специальным правилом считается за деструктивное (с. 163)
и использует определённый маркер взрыва (малый маркер, маркер большого взрыва, огромного и т.д.). Поместите соответствующий маркер, сделайте бросок на смещение и узнайте,
какой причинён урон (с. 158). При распределении ранений всегда считайте, что выстрел идёт из самого центра маркера, как и в случае с орудиями для навесной стрельбы.
Маркер вихревого оружия не удаляется из игры после отыгрыша урона. Оставьте его на столе. Маркер считается за непроходимый ландшафт, пока остаётся в игре.
В начале каждого последующего хода игрока (то есть любого игрока), маркер смещается на 2D6" (используйте маленькую стрелку, если выпадает Hit!). Если выпадает дубль, маркер удаляется из игры. Любая боевая единица, оказавшаяся под новой позицией маркера, получает попадание. Отыграйте урон, как описано выше.'
    ...
    specialRuleName: 'Zealot',
    visibleName: 'Zealot',
    textEng : 'A unit containing one or more models with the Zealot special rule automatically passes
